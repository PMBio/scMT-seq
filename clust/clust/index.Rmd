---
title: Joint clustering scBS14 and scBS15
date: 151019
output:
  html_document:
    toc: yes
...
opts$colors <- c(
  ...
  'bs_serum_bulk'='aquamarine4',
  'mt_serum'='brown2',
  'mt_serum_bulk'='brown',
  'mt3_2i'='royalblue',
  'mt3_serum'='navyblue',
  'mt3_2i_bulk'='steelblue1',
  'mt3_serum_bulk'='steelblue',
...
```{r eval=F}
dat$m14$samples %>% group_by(type) %>% summarise(n=n())
dat$m15$samples %>% group_by(type) %>% summarise(n=n())
dat$m15P3$samples %>% group_by(type) %>% summarise(n=n())
```

```{r}
d <- dat$samples
h <- grepl('bulk', as.vector(d$sample))
h <- d[h,]
```
...
plot_pca_vec <- function(pc_vec, x=1, y=2) {
  ...
  p <- ggplot(t, aes(x=pcx, y=pcy)) +
    geom_point(aes(color=stype, shape=type, size=bulk)) +
