...
opts$name <- 'gene_body'
opts$select_by <- 'met_var'
opts$n <- 300

opts$clust <- list()
opts$clust$c1 <- c('C05', 'B05', 'D05', 'E01', 'H03', 'E09', 'C07', 'F01', 'B10', 'F07', 'C02', 'B07')
opts$clust$c2 <- c('G02', 'E07', 'F03', 'C03', 'A07', 'C01', 'A06', 'A04', 'A02', 'A03', 'G07', 'C10', 'H01', 'D01', 'G09', 'E05', 'G01')
...
```{r eval=F}
write.csv(dclust$e, 'fig1e_expr.csv')
write.csv(dclust$m, 'fig1e_met.csv')
write.csv(dclust$r, 'fig1e_cor.csv')
```
...
```{r eval=F}
pdf(file='fig_heat_met.pdf', width=10, height=10)
pm <- plot_heat_met(dclust)
dev.off()
```
...
```{r eval=F}
pdf(file='fig_heat_expr.pdf', width=10, height=10)
pe <- plot_heat_expr(dclust, Rowv=pm$rowDendrogram)
dev.off()
```
...
```{r eval=F}
pdf(file='fig_heat_tracks.pdf', width=1.5, height=10)
plot_tracks(data_track(dclust, pm))
dev.off()
```
