...
# opts$samples_file <- '../data/samples/samples_meta.csv'
```
...
dat$met <- readRDS(opts$met$file) %>% mutate(sample=factor(sample, label=format_sample(levels(sample))))
```

```{r}
plot_pca_vec <- function(pc_vec, x=1, y=2, color=F) {
  t <- data.frame(sample=factor(rownames(pc_vec)),
    pcx=pc_vec[,x], pcy=pc_vec[,y])
  p <- ggplot(t, aes(x=pcx, y=pcy)) + geom_point(aes(color=sample)) +
...
pe <- pca(e)
p1 <- plot_pca_vec(pe$vec) + theme_pub()
p2 <- plot_pca_val(pe$val) + theme_pub()
```
...
```{r}
m <- dat$met %>% filter(name == 'gene_body') %>% select(id_, sample, met) %>%
  spread(sample, met) %>% select(-id_) %>% as.matrix
mi <- m %>% impute
pm <- pca(mi)
p1 <- plot_pca_vec(pm$vec) + theme_pub()
p2 <- plot_pca_val(pm$val) + theme_pub()
```
