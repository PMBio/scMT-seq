...
cmp$r <- dat$em %>% group_by(name, sample) %>%
  do(wtd_cor(.$expr, .$met, .$weight)) %>% ungroup %>%
  group_by(name) %>% mutate(p_adj=p.adjust(p, method='fdr')) %>% ungroup
```
...
```{r fig.height=10}
d <- cmp$r %>% group_by(name) %>% summarise(r_mean=mean(r, na.rm=T))
d <- cmp$r %>% inner_join(d, by='name')

p <- ggplot(d, aes(x=name, y=r)) +
  geom_hline(yintercept=0, color='darkgrey', linetype='dashed') +
  geom_boxplot(aes(fill=r_mean), alpha=0.4, outlier.size=0) +
  geom_point(position=position_jitter(w=0.1), size=1) +
  scale_fill_gradient2(low='darkblue', mid='white', high='darkred', midpoint=0.0) +
  xlab('') + ylab('r') + coord_flip() +
  theme_pub() +
  theme(legend.position='bottom') + guides(color=guide_legend(ncol=5))
print(p)
```
