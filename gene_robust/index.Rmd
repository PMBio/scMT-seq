...
cmp$st <- d %>% group_by(name, keep, trial) %>%
  summarise(
    ...
    sig=sum(p_adj.y <= opts$alpha) - sum(p_adj.x <= opts$alpha),
...
p <- ggplot(cmp$s, aes(x=name, y=mse_mean)) +
  ...
  guides(fill=guide_legend(title='Fraction of cels')) +
  coord_flip() +
  theme_pub()
print(p)
```
...
```{r eval=F}
ggsave(p, file='fig_rmse.pdf', width=8, height=7)
```

```{r fig.width=8, fig.height=7}
p <- ggplot(cmp$s, aes(x=name, y=sig_mean)) +
  geom_bar(aes(fill=keep), stat='identity', position='dodge') +
  xlab('') + ylab('Reduction in # significant correlations') +
  guides(fill=guide_legend(title='Fraction of cels')) +
  coord_flip() +
  theme_pub()
print(p)
