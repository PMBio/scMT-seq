...
```{r opts, echo=T}
opts <- list()
opts$fil$cov <- 30 # Minimum number of cells with at least one count
opts$fil$top_var <- 0.75 # Fraction of most variable sites to be selected
opts$fil$min_var <- 10 # Minimum variability
opts$adjust <- F
opts$scale_adjust <- T
...
# Filename Seqmonk methylation report
opts$rates_files <- sapply(opts$seqmonk_dirs, function(x) file.path(x, 'rates.txt'))
# Filename Seqmonk weights report
opts$rates_files <- c('./cgi_rates.txt')
opts$weights_files <- c('./cgi_weights.txt')
opts$out_base <- './data'
